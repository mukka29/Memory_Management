...
static double rand_d(const double x, const double y){
	return (x + ((y-x) / (double)rand()));
}

static int gen_weighted_addr(){
	int i;
	const double w = rand_d(user->WT[0], user->WT[PTBL_SIZE-1]);
...
int main(const int argc, char * const argv[]){
	...
	while(!stop){
		...
		//check for stop on every 1000 references
		if(++ref_count > 1000){
			if(critical_lock() < 0){
				break;
			}

			if(data->stop == 1){	//if master wants us to stop
				stop = 1;
				critical_unlock();
				break;
			}
