...
//Find a frame to be evicted, based on LRU
...
static void ptbl_reset(struct page *ptbl, struct frame * ftbl){
  int i;
	for(i=0; i < PTBL_SIZE; i++){

		struct page *vp = &ptbl[i];
...
static void deny_all(){
  ...
  //deny all blocked reference requests
  for(i=0; i < RUNNING_MAX; i++){
...
static int mem_addr_fault(struct user_pcb * usr){
  ...
  if(f >= 0){
	  ...
	  struct page * vp = &data->users[fr->u].PT[fr->p];
...
static int schedule_dispatch(){
  ...
  if(usr->ref.req == MEM_READ){ //if user wants to MEM_READ
    ...
    if(usr->ref.state == REF_ACCEPT){
      ...
      fr->state = FR_DIRTY;	//we have written to frame, mark it dirty
